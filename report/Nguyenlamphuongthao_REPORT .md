...
- **Code Highlights**:
  ...
  - Tôi sử dụng bộ `test prompts` trong `tests/run_eval.py` để so sánh `Chatbot Baseline` và `Hybrid Flow` trên 5 tình huống: chào hỏi, truy vấn có trong Local DB, truy vấn ngoài DB cần Wikipedia + OpenLibrary, truy vấn sách nước ngoài, và edge case sách không tồn tại.
...
- **Solution**: Tôi xử lý ở 3 lớp.
  ...
  Sau khi tối ưu, kết quả Gemini trong `report/eval_results_gemini.txt` cho thấy các case cần tool như TC3 và TC4 đã hoàn thành được thay vì chỉ timeout. Riêng local model vẫn còn timeout ở nhiều case, điều này phản ánh hạn chế của provider chứ không còn là lỗi cấu trúc tool như version đầu.
...
1.  **Reasoning**: `Thought` giúp agent phân rã bài toán thành từng bước có kiểm soát. Với chatbot thường, model trả lời trực tiếp từ kiến thức sẵn có nên hay suy đoán hoặc trả lời tổng quát. Với ReAct, `Thought` đóng vai trò lập kế hoạch: trước tiên lấy summary ở Wikipedia, sau đó lấy Work ID, rồi subject, rồi mới gợi ý sách tương tự. Điều này đặc biệt quan trọng ở những câu hỏi tổng hợp như *Tắt đèn* hoặc *War and Peace*, nơi một câu trả lời đúng cần ghép nhiều nguồn.
2.  **Reliability**: Agent không phải lúc nào cũng tốt hơn chatbot. Trong `report/eval_results_gemini.txt`, TC2 cho thấy Hybrid nhanh hơn vì tận dụng Local DB lookup. Nhưng trong `report/eval_results_local.txt`, nhiều case hybrid bị timeout 24-32 giây trong khi chatbot trả lời ngay là "không có thông tin". Điều đó cho thấy khi provider yếu, mạng chậm, hoặc prompt không được tuân thủ tốt, agent có thể tệ hơn chatbot về cả tốc độ lẫn độ ổn định.
